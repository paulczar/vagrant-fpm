...
cat > /tmp/elasticsearch-post-install.sh << EOF 
#!/bin/sh
...
chmod 755 /tmp/elasticsearch-post-install.sh

cat >  /tmp/elasticsearch-pre-uninstall.sh << EOF
#!/bin/sh
...
chmod 755 /tmp/elasticsearch-pre-uninstall.sh

cd /vagrant/rpms
fpm -s dir -t rpm -d 'java-1.7.0-openjdk' --post-install "/tmp/elasticsearch-post-install.sh" \
		--pre-uninstall "/tmp/elasticsearch-pre-uninstall.sh" -n "elasticsearch" -v $VERSION $ES_DIR
